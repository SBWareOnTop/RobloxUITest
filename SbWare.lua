...
local requestsDisabled = true --getgenv and getgenv().DISABLE_SBWare_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local SBWareFolder = "SBWare"
local ConfigurationFolder = SBWareFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		SBWareOpen = {Type = 'bind', Value = 'K', Name = 'SBWare Keybind'},
		-- buildwarnings
		-- SBWareprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.SBWareOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(SBWareFolder) then
				if isfile and isfile(SBWareFolder..'/settings'..ConfigurationExtension) then
					file = readfile(SBWareFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"SBWareOpen":{"Value":"K","Type":"bind","Name":"SBWare Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"SBWare Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('SBWare had an issue accessing configuration saving capability.')
		end
	end
end
...
local SBWare = useStudio and script.Parent:FindFirstChild('SBWare') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local SBWareDestroyed = false -- True when SBWareLibrary:Destroy() is called

repeat
	if SBWare:FindFirstChild('Build') and SBWare.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('SBWare | Build Mismatch')
		print('SBWare may encounter issues as you are running an incompatible interface version ('.. ((SBWare:FindFirstChild('Build') and SBWare.Build.Value) or 'No Build') ..').\n\nThis version of SBWare is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, SBWare = SBWare, useStudio and script.Parent:FindFirstChild('SBWare') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

SBWare.Enabled = false

if gethui then
	SBWare.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(SBWare)
	SBWare.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	SBWare.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SBWare.Name and Interface ~= SBWare then
			Interface.Enabled = false
			Interface.Name = "SBWare-Old"
		end
	end
elseif not useStudio then
...
if SBWare.AbsoluteSize.X < minSize.X and SBWare.AbsoluteSize.Y < minSize.Y then
...
local Main = SBWare.Main
local MPrompt = SBWare:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = SBWare:FindFirstChild('Drag')
...
SBWare.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SBWareOnTop/RobloxUITest/refs/heads/main/icons.Lua')
-- Variables
...
local Notifications = SBWare.Notifications

local SelectedTheme = SBWareLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = SBWareLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	SBWare.Main.BackgroundColor3 = SelectedTheme.Background
	SBWare.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	SBWare.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	SBWare.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	SBWare.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	SBWare.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	SBWare.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		SBWare.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		SBWare.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(SBWare:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("SBWare | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("SBWare | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('SBWare had an issue decoding the configuration file, please try delete the file and reopen SBWare.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(SBWareLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("SBWare | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--SBWareLibrary:Notify({Title = "SBWare Flags", Content = "SBWare was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(SBWareLibrary.Flags) do
...
function SBWareLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('SBWare | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			SBWareLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			SBWareLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "SBWareOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(SBWareFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('SBWare Settings', 0, true)

	if TabList['SBWare Settings'] then
		TabList['SBWare Settings'].LayoutOrder = 1000
	end

	if Elements['SBWare Settings'] then
		Elements['SBWare Settings'].LayoutOrder = 1000
	end
...
function SBWareLibrary:CreateWindow(Settings)
	print('creating window')
	if SBWare:FindFirstChild('Loading') then
		if getgenv and not getgenv().SBWareCached then
			SBWare.Enabled = true
			SBWare.Loading.Visible = true

			task.wait(1.4)
			SBWare.Loading.Visible = false
		end
	end

	if getgenv then getgenv().SBWareCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				SBWareLibrary:Notify({Title = 'Build Mismatch', Content = 'SBWare may encounter issues as you are running an incompatible interface version ('.. ((SBWare:FindFirstChild('Build') and SBWare.Build.Value) or 'No Build') ..').\n\nThis version of SBWare is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "SBWare"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "SBWare Interface Suite" then
		LoadingFrame.Version.Text = "SBWare UI"
	end
	...
	if not Settings.DisableSBWarePrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				SBWareLibrary:Notify({
					Title = "SBWare Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(SBWareFolder.."/Discord Invites") then
			makefolder(SBWareFolder.."/Discord Invites")
		end

		if isfile and not isfile(SBWareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(SBWareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"SBWare RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(SBWareFolder.."/Key System") then
			makefolder(SBWareFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("SBWare | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with SBWare specific development.')
				end
			end
		end
		...
		if isfile and isfile(SBWareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(SBWareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SBWareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						SBWareLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called SBWareLibrary:Destroy()
				if SBWareDestroyed then
					return
				end
				if not Success then
					...
					print("SBWare | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with SBWare specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= SBWareLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SBWareLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			SBWare.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("SBWare | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with SBWare specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					SBWareLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("SBWare | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with SBWare specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SBWareLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("SBWare | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with SBWare specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SBWareLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= SBWareLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("SBWare | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with SBWare specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						SBWareLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("SBWare | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with SBWare specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SBWareLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			SBWareLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			SBWareLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('SBWare had an issue creating settings.') end
...
function SBWareLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function SBWareLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function SBWareLibrary:Destroy()
	SBWareDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['SBWare Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "SBWareOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function SBWareLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				SBWareLibrary:Notify({Title = "SBWare Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			SBWareLibrary:Notify({Title = "SBWare Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('SBWare Configurations Error | '..tostring(result))
			SBWareLibrary:Notify({Title = "SBWare Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = SBWareLibrary:CreateWindow({
	--	Name = "SBWare Example Window",
	--	LoadingTitle = "SBWare Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using SBWare may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like SBWare to get the key from
	...
	--local thoptions = {}
	--for themename, theme in pairs(SBWareLibrary.Theme) do
...
task.delay(4, function()
	SBWareLibrary.LoadConfiguration()
